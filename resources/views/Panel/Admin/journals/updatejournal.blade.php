...
<form action="{{URL('adminupdatejournal',$journal->Journal_id)}}" 
method="POST" class="expertform"id="editform"enctype="multipart/form-data">
@csrf
<h1>Update journal</h1>

<div hidden class="m-1"style="height:8.5em;padding:.5em; margin-top:0;">
<label for="date" style="margin-top:;"class=" text-light form-label"><strong>Student ID</strong> </label>
<input readonly type="text" class="bg-dark text-light form-control w-100" id="name" value="{{$journal->user_id}}"  name="student_id">    
</div>
...
<select  style="border:1px solid white;background:linear-gradient(to right,#141E30,#243B55);
    width:auto;height:2em;border-radius:.3em;" name="view_content"class="w-10 bg-dark text-light form-select"  aria-label="Default select example">
